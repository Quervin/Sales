...
namespace Sales.WEB.Auth
{
    public class AuthenticationProviderJWT : AuthenticationStateProvider, ILoginService
    {
        ...
        public override async Task<AuthenticationState> GetAuthenticationStateAsync()
        {
            ...
            return BulidAuthenticationState(token.ToString()!);
        }

        private AuthenticationState BulidAuthenticationState(string token)
        {
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("bearer", token);
            var claims = ParseClaimsFromJWT(token);
            return new AuthenticationState(new ClaimsPrincipal(new ClaimsIdentity(claims, "jtw")));
        }

        private IEnumerable<Claim>? ParseClaimsFromJWT(string token)
        {
            var jwtSecurityTokenHandler = new JwtSecurityTokenHandler();
            var unserializaedToken = jwtSecurityTokenHandler.ReadJwtToken(token);
            return unserializaedToken.Claims;
        }

        public async Task  LoginAsync(string token)
        {
            await _jSRuntime.SetLocalStorage(_tokenKey, token);
            var authState = BulidAuthenticationState(token);
